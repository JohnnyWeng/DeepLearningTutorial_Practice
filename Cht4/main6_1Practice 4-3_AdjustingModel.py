...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        # 6 -> 8
        # 3: input channel, 8: output channel
        # input: RGB (3), output: 8
        # We keep increasing the channel: 3 -> 8 -> 16...
        self.conv1 = nn.Conv2d(3, 8, 5) #(in, out, kernel)
        # Max pooling: decease to an half
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(8, 16, 5)
        # You can see the input has to be same as "previous output". 120 -> 84 -> 10
    ...
    def forward(self, x):
        ...
        # The above is only difine the net. Here we actually utilize the pooling layer with the conv.
        # Please look at the summary of the model, the pooling reduce to a half after pooling.
        x = self.pool(F.relu(self.conv1(x))) # 先 convolution, 再做 relu, 然後再 pooling
