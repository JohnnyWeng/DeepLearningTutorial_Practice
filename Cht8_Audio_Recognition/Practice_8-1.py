...
def generate_noisy_sine_wave(freq, sample_rate=16000, duration=1, noise_level=0.05):
    ...
    # torch.randn_like：When you already have a reference tensor and want to generate a random tensor that is the same shape as it.
    noise = noise_level * torch.randn_like(waveform)
    # add noise to add some difficulties.
...
class SimpleAudioDataset():
    def __init__(self, transform=None):
        ...
        for label, freq in enumerate(frequencies):
            for _ in range(50):  # Generate 50 samples per frequency
                sine_wave = generate_noisy_sine_wave(freq)
                self.data.append(sine_wave)
                self.labels.append(label)  # Assign label based on index in the list
...
class AudioTransform:
    def __init__(self):
        self.mel_spectrogram = MelSpectrogram(
            sample_rate=16000,
            n_mels=32, # The numbers of filter in Mel  spectrum -> 32 filter to recognize.
...
class SimpleAudioModel(nn.Module):
    def __init__(self, sample_input):
        ...
        self.fc2 = nn.Linear(64, 8).to(device)  # Change output to 8 classes
...
def train_model(model, train_loader, criterion, optimizer):
    model.train()
    # Each time of training we give the model a batch of data.
    for inputs, labels in train_loader:
        inputs, labels = inputs.to(device), labels.to(device)
        optimizer.zero_grad() # It zero out the gradients at the beginning of each loader looping.
...
if __name__ == "__main__": # Main method
    # Here we will have 3 major process: 1. training 2.evaludate 3.predicting.
    transform = AudioTransform()
    # The dataset (8)
    dataset = SimpleAudioDataset(transform=transform)

    # Split dataset into training and validation sets
    # This is another way of splitting the dataset.
    ...
    for epoch in range(num_epochs):
        ...
        # You can see that the accuracy increases after each time the training with a batches.
    ...
    for i, freq in enumerate(frequencies):
        ...
        print(f"Frequency: {freq} Hz, Predicted Label: {predicted_label}")
