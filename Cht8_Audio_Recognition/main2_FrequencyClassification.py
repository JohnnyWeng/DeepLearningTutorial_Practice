...
class SimpleAudioDataset(Dataset):
    def __init__(self, transform=None):
        ...
        self.labels = []
        for _ in range(50):
            sine_wave1 = generate_noisy_sine_wave(440) # 看 ppt, 波型比 880 鬆
            sine_wave2 = generate_noisy_sine_wave(880) # 拉的音, 比 440 音更高
            self.data.append(sine_wave1)
            # 波型結果是 0~1 之間
            self.labels.append(0) # 440 的label 是 0
            self.data.append(sine_wave2)
            self.labels.append(1) # 880 的label 是 1
    def __len__(self):
...
def plot_waveform(waveform, sample_rate=16000, freq=440, title="Waveform"):
    if waveform.dim() > 1:
        waveform = waveform.squeeze() # 維度再縮回來
...
if __name__ == "__main__": # Main method
    ...
    plot_waveform(test_waveform440, sample_rate=16000, freq=400, title="Waveform of 440 Hz Signal")
    # Plot the waveform of 800 Hz
    #880 比較密集
    plot_waveform(test_waveform880, sample_rate=16000, freq=800, title="Waveform of 880 Hz Signal")