...
class DQN(object):
    ...
    def choose_action(self, x):  # Defining the Action Selection Function (x is the state)
        # unsqueeze: increase the dimention
        ...
        if np.random.uniform() < EPSILON:
            actions_value = self.eval_net.forward(
                x)  # By entering the state x, the action value is obtained by forward propagation
    ...
    def learn(self):
        if self.learn_step_counter % TARGET_REPLACE_ITER == 0:  # Triggered at the beginning, then every 100 steps
            self.target_net.load_state_dict(
                self.eval_net.state_dict())  # Assign the parameters of the evaluation network to the target network
        self.learn_step_counter += 1
        sample_index = np.random.choice(MEMORY_CAPACITY,
                                        BATCH_SIZE)  # 32 numbers are randomly drawn within [0, 2000) and may be repeated
...
for i in range(400):
    ...
    while True:  # Start an episode (Each cycle represents a step)
        ...
        r2 = (env.theta_threshold_radians - abs(
            theta)) / env.theta_threshold_radians - 0.5  # The more positive the pillars(柱子), the better
        new_r = r1 + r2
